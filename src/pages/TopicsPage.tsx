...
export function TopicsPage() {
	...
	return <section>
		...
		{loading ? <div className="empty-state">Loading topics…</div> : error ? <div className="empty-state">{error}</div> : <div className="topic-grid">{topics.map((topic) => <Link className="large-topic-card" to={`/topics/${topic.id}`} key={topic.id}><span className="topic-icon violet"><BookOpen size={20} /></span><h2>{topic.name}</h2><p>{topic.solved} / {topic.total} solved · {topic.description ?? 'Questions to explore'}</p><span className="card-arrow"><ChevronRight size={17} /></span></Link>)}</div>}
