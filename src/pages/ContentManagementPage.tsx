		...
		<section>
			...
			{(section === 'questions' || section === 'topics') && <div className="management-grid">
				<div className="panel management-list">
					...
					{section === 'questions'
						? questions.map((item) => <ManagementRow key={item.id} title={item.title} detail={`${item.topicId} — ${item.type.replace(/_/g, ' ')}`} onEdit={() => setEditing({ kind: 'question', title: item.title })} />)
...
function ModerationPanel({ items, queue, onAction, notice }: { items: ApiQuestion[]; queue: 'reviewer' | 'admin'; onAction: (id: string, action: string) => void; notice: string }) {
	return <div className="panel moderation-panel"><div className="panel-heading"><div><span className="eyebrow">{queue === 'reviewer' ? 'Reviewer queue' : 'Admin queue'}</span><h2>{items.length ? `${items.length} questions waiting` : 'Queue is clear'}</h2></div></div>{items.length === 0 ? <div className="empty-state">There are no questions waiting for this queue.</div> : items.map((item) => <div className="moderation-row" key={item.id}><span><strong>{item.title}</strong><small>{item.status.replace(/_/g, ' ')} — {item.type.replace(/_/g, ' ')}</small></span><span className="row-actions">{queue === 'reviewer' && item.status === 'submitted' && <button className="secondary-button" onClick={() => onAction(item.id, 'start_review')}>Start review</button>}{queue === 'reviewer' && item.status === 'in_review' && <><button className="secondary-button" onClick={() => onAction(item.id, 'request_changes')}>Request changes</button><button className="secondary-button" onClick={() => onAction(item.id, 'reject')}>Reject</button></>}{queue === 'admin' && item.status === 'in_review' && <button className="primary-button" onClick={() => onAction(item.id, 'approve')}>Approve</button>}{queue === 'admin' && item.status === 'approved' && <button className="primary-button" onClick={() => onAction(item.id, 'publish')}>Publish</button>}</span></div>)}{notice && <p className="form-notice">{notice}</p>}</div>
}
